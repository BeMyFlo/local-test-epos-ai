...
7 of 15 built (P1 home trio + P2 about pair + P3 film pair); the remaining 8 land in later S3 task runs.
...
docker exec ${PROJECT_ID} wp eval 'foreach (["home-hero","home-who-we-are","home-featured-works","about-story","about-pillars","film-intro","film-tiers"] as $b) { $n="ai-zippy/".$b; printf("%-32s %s\n", $n, WP_Block_Type_Registry::get_instance()->is_registered($n) ? "registered" : "MISSING"); }' --allow-root

# No PHP fatals (error.log is a /dev/stderr symlink in this image family)
docker logs ${PROJECT_ID} 2>&1 >/dev/null | tail -50
```

Expected: 7× `registered`, error stream free of new PHP Fatal/Warning. (Frontend render +
