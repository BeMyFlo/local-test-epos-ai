			...
			<InspectorControls>
				<PanelBody title="Content">
					...
					<TextareaControl
						...
						onChange={(v) => setAttributes({ para1: v })}
					/>
					<TextareaControl
						label="Paragraph 2"
						value={attributes.para2}
						onChange={(v) => setAttributes({ para2: v })}
					/>
					<TextareaControl
						label="Paragraph 3"
						value={attributes.para3}
						onChange={(v) => setAttributes({ para3: v })}
					/>
					<TextControl
						label="Pillars heading"
						value={attributes.pillarsTitle}
						onChange={(v) => setAttributes({ pillarsTitle: v })}
					/>
				</PanelBody>
				{cards.map((card, i) => (
